...
import { transactions, revenue7d, fmtCurrency } from "@/lib/mock-data";
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 p-6 md:grid-cols-4">
        <StatCard label="Total (7d)" value={fmtCurrency(total)} icon={<DollarSign className="h-5 w-5" />} delta="+18.4% WoW" accent="primary" />
        <StatCard label="Wallet top-ups" value={fmtCurrency(topups)} icon={<Wallet className="h-5 w-5" />} accent="success" />
        <StatCard label="Voucher sales" value={fmtCurrency(vouchers)} icon={<CreditCard className="h-5 w-5" />} accent="info" />
        <StatCard label="Ad revenue" value={fmtCurrency(adsRev)} icon={<TrendingUp className="h-5 w-5" />} accent="warning" />
      </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenue7d}>
                ...
                <Line type="monotone" dataKey="topups" stroke="oklch(0.82 0.16 195)" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="vouchers" stroke="oklch(0.78 0.18 150)" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="ads" stroke="oklch(0.82 0.17 80)" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Panel>
            ...
            <TableBody>
              {transactions.map((t) => (
                <TableRow key={t.id} className="font-mono text-xs">
                  ...
                  <TableCell><span className="rounded bg-muted px-1.5 py-0.5 text-[10px] uppercase">{t.type}</span></TableCell>
                  <TableCell className="text-muted-foreground uppercase">{t.method}</TableCell>
                  <TableCell className="text-right text-success">+{fmtCurrency(t.amount)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
