...
import { users, fmtCurrency } from "@/lib/mock-data";
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 p-6 md:grid-cols-3">
        <StatCard label="Active wallets" value={String(users.filter((u) => u.status === "active").length)} icon={<UserCheck className="h-5 w-5" />} accent="success" />
        <StatCard label="Wallet float" value={fmtCurrency(float)} icon={<Wallet className="h-5 w-5" />} accent="primary" />
        <StatCard label="Low balance (<10)" value={String(lowBal)} icon={<AlertCircle className="h-5 w-5" />} accent="warning" />
      </div>
            ...
            <TableBody>
              {users.map((u) => (
                <TableRow key={u.id} className="font-mono text-xs">
                  ...
                  <TableCell className="text-right text-muted-foreground">{fmtCurrency(u.lifetimeSpend)}</TableCell>
                  <TableCell className="text-right">{(u.dataUsedMb / 1024).toFixed(2)} GB</TableCell>
